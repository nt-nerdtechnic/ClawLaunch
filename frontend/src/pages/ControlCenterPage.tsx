...
export const ControlCenterPage: React.FC<ControlCenterPageProps> = ({ onRefreshSnapshot }) => {
  const { t } = useTranslation();
  const { snapshot, eventQueue } = useStore();
  ...
  const tasks = useMemo<TaskItem[]>(() => {
    ...
    return snapshot.tasks.map((item) => ({
      id: String(item.id || ''),
      title: String(item.title || ''),
      status: (String(item.status || '').toLowerCase() as TaskStatus) || 'todo',
  ...
  const queueItems = useMemo<QueueItem[]>(() => {
    return eventQueue.map((item) => ({
      id: String(item.id || ''),
      title: String(item.title || ''),
      detail: String(item.detail || ''),
      severity: item.level === 'action-required' ? 'critical' : item.level === 'warn' ? 'warn' : 'info',
      status: String(item.status || '').toLowerCase() === 'acked' ? 'acked' : 'pending',
  ...
  }, [tasks, pendingQueue.length]);

  const threadSummary = useMemo(() => {
    const running = overview.runningTasks;
    const blocked = overview.blockedTasks;
    const completed = overview.doneTasks;
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
          ...
          <div className="text-2xl font-black">{overview.taskCount}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
          <div className="text-xs text-slate-500">{t('controlCenter.kpi.pendingQueue')}</div>
          <div className="text-2xl font-black">{overview.pendingQueue}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
            ...
            {tasks.map((item) => (
              <div key={item.id} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950/40 p-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500">{item.status} · {item.scope || '-'}</div>
                </div>
                <select
                  value={item.status}
