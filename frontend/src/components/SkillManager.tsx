...
const INITIAL_SKILLS = [
  { id: 'browser', name: 'Browser Automation', desc: '控制瀏覽器執行網頁操作', enabled: true, category: 'Core', details: '支持 Playwright 與 Puppeteer 雙引擎。' },
  { id: 'coding', name: 'Coding Agent', desc: '呼叫 Claude Code 執行代碼開發', enabled: true, category: 'Dev', details: '深度整合指令碼生成與 Git 自動提交功能。' },
  { id: 'search', name: 'Web Search', desc: '使用 Brave API 進行全網搜索', enabled: false, category: 'Core', details: '需要配置 BRAVE_API_KEY 以啟用實時搜索。' },
  { id: 'market', name: 'Market Sentinel', desc: '監測特定品牌與產品公關風險', enabled: true, category: 'Intel', details: '定期爬取社群媒體與新聞網站進行情感分析。' },
  { id: 'cron', name: 'Cron Guardian', desc: '監管背景排程與心跳機制', enabled: true, category: 'Ops', details: '管理 Moltbot 的週期性自定義任務。' },
];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 pb-20">
      <div className="flex items-center justify-between">
        ...
        <div className="text-[10px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-3 py-1 rounded-full font-mono uppercase">
            Active: {skills.filter(s => s.enabled).length} / {skills.length}
        </div>
      </div>
            ...
            <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => setExpandedId(expandedId === skill.id ? null : skill.id)}>
                ...
                <div className="flex items-center space-x-6">
                    <div 
                        ...
                        className={`w-11 h-6 rounded-full p-1 transition-all duration-300 cursor-pointer ${skill.enabled ? 'bg-emerald-500' : 'bg-slate-800'}`}
                    >
                        <div className={`w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-300 ${skill.enabled ? 'translate-x-5' : 'translate-x-0'}`}></div>
                    </div>
