...
const PERSISTENT_CONFIG_DIR = path.join(app.getPath('appData'), app.getName().replace(/ /g, '-'));
...
process.stderr?.on('error', (err) => { if (err.code === 'EPIPE')
    return; });
process.on('uncaughtException', (err) => { if (err.code === 'EPIPE')
    return; throw err; });
...
const DEFAULT_GATEWAY_WATCHDOG_OPTIONS = {
    autoRestart: false,
    maxRestarts: 5,
    baseBackoffMs: 1000,
};
const DEFAULT_GATEWAY_HTTP_WATCHDOG_OPTIONS = {
    enabled: false,
    healthCheckCommand: '',
    restartCommand: '',
    intervalMs: 15000,
...
const emitShellStdout = (data, source = 'stdout') => {
    sendToRenderer('shell:stdout', { data, source });
};
const buildTerminalLaunchScript = (command, title = 'OpenClaw Gateway Auto-Restart') => {
...
const runGatewayHttpWatchdogCheck = async () => {
    if (gatewayHttpWatchdog.checking)
        return;
    if (!gatewayHttpWatchdog.options.enabled)
        return;
    if (Date.now() < gatewayHttpWatchdog.suppressChecksUntil)
        return;
    ...
    if (!healthCheckCommand || !restartCommand)
        return;
    ...
    try {
        ...
        emitShellStdout(`[gateway-http-watchdog] health check failed (${gatewayHttpWatchdog.consecutiveFailures}/${gatewayHttpWatchdog.options.failThreshold}) code=${String(healthRes.code)} stdout=${failStdout ? 'non-empty' : 'empty'} stderr=${failStderr ? 'non-empty' : 'empty'}\n`, 'stderr');
        ...
        if (gatewayHttpWatchdog.restartAttempts >= gatewayHttpWatchdog.options.maxRestarts) {
            emitShellStdout(`[gateway-http-watchdog] max restart attempts reached (${gatewayHttpWatchdog.options.maxRestarts}), stop restarting\n`, 'stderr');
            return;
        }
        gatewayHttpWatchdog.restartAttempts += 1;
        emitShellStdout(`[gateway-http-watchdog] restart attempt ${gatewayHttpWatchdog.restartAttempts}/${gatewayHttpWatchdog.options.maxRestarts} via macOS Terminal\n`, 'stdout');
...
const startGatewayHttpWatchdog = (options) => {
    const nextOptions = {
    ...
    gatewayHttpWatchdog.checking = false;
    emitShellStdout(`[gateway-http-watchdog] started (interval=${nextOptions.intervalMs}ms, threshold=${nextOptions.failThreshold})\n`, 'stdout');
    gatewayHttpWatchdog.timer = setInterval(() => {
        void runGatewayHttpWatchdogCheck();
    }, nextOptions.intervalMs);
    void runGatewayHttpWatchdogCheck();
};
const launchGatewayViaTerminal = async (command) => {
    const osascriptCmd = buildTerminalLaunchScript(command);
    const res = await runShellCommand(osascriptCmd);
    return (res.code ?? 1) === 0;
};
const spawnWatchedGatewayProcess = (command) => {
    ...
    child.on('exit', async (code, signal) => {
        ...
        emitShellStdout(`[gateway-watchdog] process exited unexpectedly (code=${String(code)}, signal=${String(signal)})\n`, 'stderr');
        ...
        if (gatewayWatchdog.restartAttempts >= gatewayWatchdog.options.maxRestarts) {
            emitShellStdout(`[gateway-watchdog] max restart attempts reached (${gatewayWatchdog.options.maxRestarts}), stop restarting\n`, 'stderr');
            return;
        }
        gatewayWatchdog.restartAttempts += 1;
        const delayMs = Math.min(gatewayWatchdog.options.baseBackoffMs * 2 ** (gatewayWatchdog.restartAttempts - 1), 30000);
        emitShellStdout(`[gateway-watchdog] restart attempt ${gatewayWatchdog.restartAttempts}/${gatewayWatchdog.options.maxRestarts} in ${delayMs}ms\n`, 'stdout');
        clearGatewayRestartTimer();
...
const normalizeConfigDir = (rawPath) => {
...
const pickFirst = (input, keys, fallback = '') => {
...
const normalizeBudgetSummary = (budgetSummary) => {
...
const normalizeReadModelSnapshot = (snapshot) => {
    ...
    for (const status of statuses) {
        if (status.sessionKey)
            statusMap.set(status.sessionKey, status);
    }
    const sessions = sessionsRaw.map((item) => {
        ...
        return {
            ...
            tokensIn: normalizeNumber(pickFirst(item, ['tokensIn', 'inputTokens', 'tokens_in', 'usageIn'], mappedStatus?.tokensIn || 0), 0),
            tokensOut: normalizeNumber(pickFirst(item, ['tokensOut', 'outputTokens', 'tokens_out', 'usageOut'], mappedStatus?.tokensOut || 0), 0),
            cost: normalizeNumber(pickFirst(item, ['cost', 'totalCost', 'estimatedCost', 'costUsd'], mappedStatus?.cost || 0), 0),
            model: normalizeString(pickFirst(item, ['model', 'modelName'], mappedStatus?.model || '')),
            updatedAt: normalizeString(pickFirst(item, ['updatedAt', 'lastSeenAt', 'timestamp', 'createdAt'], raw.generatedAt || new Date().toISOString())),
        };
    });
    const tasks = tasksSource.map((item) => ({
        ...
        updatedAt: normalizeString(pickFirst(item, ['updatedAt', 'lastHeartbeatAt', 'createdAt', 'timestamp'], raw.generatedAt || new Date().toISOString())),
    }));
    const approvals = approvalsSource.map((item) => ({
        ...
        requestedAt: normalizeString(pickFirst(item, ['requestedAt', 'createdAt', 'timestamp'], raw.generatedAt || new Date().toISOString())),
    }));
    return {
...
const resolveTimestampFromLogEntry = (entry) => {
    return normalizeString(pickFirst(entry, ['timestamp', 'session_timestamp', 'generatedAt', 'updatedAt', 'createdAt', 'at'], ''), '');
};
const resolveTokensFromLogEntry = (entry) => {
    const tokensIn = normalizeNumber(pickFirst(entry, ['input_tokens', 'tokensIn', 'inputTokens', 'tokens_in', 'usageIn'], 0), 0);
    const tokensOut = normalizeNumber(pickFirst(entry, ['output_tokens', 'tokensOut', 'outputTokens', 'tokens_out', 'usageOut'], 0), 0);
    return { tokensIn, tokensOut };
};
const estimateUsageCost = (tokensIn, tokensOut) => ((tokensIn + tokensOut * 2) / 1_000_000) * 0.5;
const resolveCostFromLogEntry = (entry, tokensIn, tokensOut) => {
    ...
    const directCost = normalizeNumber(pickFirst(entry, ['estimatedCost', 'totalCost', 'usageCost', 'costUsd', 'usd_cost', 'cost'], NaN), NaN);
    const candidates = [usageCostFromMessage, usageCost, costObjectTotal, directCost].filter((value) => Number.isFinite(value) && value >= 0);
    if (candidates.length > 0)
        return candidates[0];
    return estimateUsageCost(tokensIn, tokensOut);
};
const resolveCostFromSessionEntry = (session, tokensIn, tokensOut) => {
    const directCost = normalizeNumber(pickFirst(session, ['estimatedCost', 'totalCost', 'usageCost', 'costUsd', 'usd_cost', 'cost'], NaN), NaN);
...
const inferProviderFromModel = (model) => {
    ...
    if (m.includes('gemini'))
        return 'Google';
    if (m.includes('llama') || m.includes('mistral') || m.includes('qwen') || m.includes('deepseek'))
        return 'OSS/Other';
    return 'Unknown';
};
const parseSessionJsonlForUsage = (content, agentId) => {
    const events = [];
    ...
    for (const line of content.split(/\r?\n/).map((l) => l.trim()).filter(Boolean)) {
        ...
        if (entry.type === 'session' && entry.id)
            currentSessionId = String(entry.id);
        if (entry.sessionId)
            currentSessionId = String(entry.sessionId);
        if (entry.message?.model)
            currentModel = String(entry.message.model);
        if (entry.type === 'message' && entry.message?.role === 'assistant' && entry.message?.usage) {
            const usage = entry.message.usage;
            const tokensIn = normalizeNumber(pickFirst(usage, ['input', 'inputTokens', 'input_tokens', 'prompt_tokens', 'promptTokens'], 0), 0);
            const tokensOut = normalizeNumber(pickFirst(usage, ['output', 'outputTokens', 'output_tokens', 'completion_tokens', 'completionTokens'], 0), 0);
            const cacheRead = normalizeNumber(pickFirst(usage, ['cacheRead', 'cache_read_input_tokens', 'cacheReadInputTokens'], 0), 0);
            const cacheWrite = normalizeNumber(pickFirst(usage, ['cacheWrite', 'cache_creation_input_tokens', 'cacheCreationInputTokens'], 0), 0);
            const cacheTokens = cacheRead + cacheWrite;
            const tokens = tokensIn + tokensOut + cacheTokens;
            if (tokens === 0)
                continue;
            const cost = normalizeNumber(usage?.cost?.total ?? usage?.cost ?? usage?.estimatedCost ?? usage?.totalCost ?? 0, 0);
            ...
            const model = entry.message?.model || currentModel || undefined;
...
// ──────────────────────────────────────────────────────────────────────────
const buildReadModelHistoryFromJsonl = (content, days = 7) => {
...
const fallbackHistoryFromSnapshot = (readModel, days = 7) => {
...
const computeTaskGovernance = (readModel, timeoutMs, nowIso) => {
...
const buildGovernanceEvents = (readModel, nowIso) => {
    const events = [];
...
const resolveRuntimeDirFromCandidates = async (candidatePaths) => {
...
const loadEventAcks = async (runtimeDir) => {
    if (!runtimeDir)
        return {};
...
const saveEventAcks = async (runtimeDir, acks) => {
    if (!runtimeDir)
        return;
...
const applyAckStateToEvents = (events, acks, nowIso) => {
    const now = new Date(nowIso).getTime();
    const activeEvents = [];
    const ackedEvents = [];
...
const parseAuditLine = (line, source) => {
    ...
    if (parsed && typeof parsed === 'object') {
        ...
        const level = severityRaw === 'error' || severityRaw === 'critical' ? 'action-required' : severityRaw === 'warn' || severityRaw === 'warning' ? 'warn' : 'info';
...
const buildAuditTimeline = async (runtimeDir, governanceEvents) => {
    const entries = governanceEvents.map((event) => ({
...
const buildDailyDigestMarkdown = (timeline) => {
    const now = new Date();
...
const buildGatewayUrlArg = (gatewayPort) => {
...
const resolveGatewayCredentials = (config) => {
...
const buildGatewayAuthArg = (credentials) => {
...
const runShellCommand = (command) => new Promise((resolve) => {
    const child = spawn(command, { shell: true });
    activeProcesses.add(child);
    let stdout = '';
    let stderr = '';
...
const isGatewayOnlineFromStatus = (statusRes) => {
    ...
    if (parsed && typeof parsed === 'object') {
        if (parsed.online === true)
            return true;
        if (parsed.gateway?.online === true)
            return true;
        if (parsed.probe?.online === true || parsed.probe?.ok === true)
            return true;
        if (typeof parsed.status === 'string' && /online|running/i.test(parsed.status))
            return true;
    }
    return false;
};
...
const parseGatewayCallStdoutJson = (rawStdout) => {
...
const pickTextFromUnknownContent = (content) => {
...
const extractLatestAssistantTextFromHistoryPayload = (payload) => {
...
const fetchLatestAssistantText = async (runtimePrefix, gatewayUrlArg, gatewayAuthArg, sessionKey, agentId) => {
    const historyParams = { sessionKey };
    if (agentId)
        historyParams.agentId = agentId;
    ...
    if (historyRes.code !== 0) {
        return { ok: false, text: '', error: historyRes.stderr || 'chat.history failed' };
    }
    const parsed = parseGatewayCallStdoutJson(historyRes.stdout);
    if (!parsed) {
        return { ok: false, text: '', error: 'chat.history returned non-JSON output' };
    }
    return { ok: true, text: extractLatestAssistantTextFromHistoryPayload(parsed), error: '' };
};
const waitForAssistantFinalByHistory = async ({ request, runtimePrefix, gatewayUrlArg, gatewayAuthArg, baseline, emitChunk, }) => {
...
function validateVersionRef(raw) {
...
function isDevServerReachable(url, timeoutMs = 600) {
...
async function writeLockFile(configPathDir) {
...
async function checkConfigPathConflict(configPathDir) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            try {
                await fs.unlink(path.join(configPathDir, entry));
            }
            catch { }
        }
    }
    ...
    for (let i = 2; i <= 9; i++) {
        ...
        if (candidateFree) {
            suggestionPath = candidate;
            break;
        }
    }
    return { conflictPid, suggestionPath };
}
async function activateConfigPath(newConfigPath) {
 ...
 */
function inferAuthChoiceFromProfile(profile) {
    ...
    if (provider === 'openai-codex')
        return 'openai-codex';
    if (provider === 'openai')
        return mode === 'oauth' ? 'openai-codex' : 'openai-api-key';
    if (provider === 'google' || provider === 'gemini')
        return mode === 'oauth' ? 'google-gemini-cli' : 'gemini-api-key';
    if (provider === 'google-gemini-cli')
        return 'google-gemini-cli';
    if (provider === 'minimax-portal')
        return 'minimax-coding-plan-global-token';
    if (provider === 'minimax')
        return 'minimax-api';
    ...
    if (provider === 'chutes')
        return 'chutes';
    if (provider === 'qwen-portal' || provider === 'qwen')
        return 'qwen-portal';
    return '';
}
/**
...
function parseOpenClawConfig(content) {
    try {
            ...
            for (const key in parsed.auth.profiles) {
                ...
                if (possibleKey && typeof possibleKey === 'string' && possibleKey.length > 5) {
                    ...
                    if (!authChoice) {
                        ...
                        if (lowKey.includes('anthropic'))
                            authChoice = 'apiKey';
                        else if (lowKey.includes('openai'))
                            authChoice = 'openai-api-key';
                        else if (lowKey.includes('gemini'))
                            authChoice = 'gemini-api-key';
                        else if (lowKey.includes('minimax'))
                            authChoice = 'minimax-api';
                        else if (lowKey.includes('deepseek') || lowKey.includes('ollama'))
                            authChoice = 'ollama';
                    }
                    break;
                }
        ...
        if (!authChoice && model) {
            ...
            if (lowModel.includes('claude'))
                authChoice = 'apiKey';
            else if (lowModel.includes('gpt'))
                authChoice = 'openai-api-key';
            else if (lowModel.includes('gemini'))
                authChoice = 'gemini-api-key';
            else if (lowModel.includes('minimax'))
                authChoice = 'minimax-api';
            else if (lowModel.includes('ollama'))
                authChoice = 'ollama';
            else if (lowModel.includes('deepseek'))
                authChoice = 'ollama';
        }
        // Final fallback
        ...
        return { apiKey: '', model: '', workspace: '', botToken: '', corePath: '', authChoice: '', providers: [], gateway: null };
    }
}
const AUTH_CHOICE_FLAG_MAPPING = {
...
const AUTH_CHOICE_PROVIDER_ALIASES = {
    apiKey: ['anthropic'],
    token: ['anthropic'],
...
const sanitizeSecret = (value) => String(value || '').replace(/\s+/g, '');
const hasCjkCharacters = (value) => /[\u3040-\u30ff\u3400-\u9fff\uf900-\ufaff]/.test(String(value || ''));
const isLikelyNaturalLanguageSentence = (value) => {
    const text = String(value || '').trim();
    if (!text)
        return false;
...
const isPlausibleMachineToken = (value) => {
    const text = String(value || '').trim();
    if (!text)
        return false;
    if (text.length < 16)
...
const normalizeConfigDirectory = (rawPath) => String(rawPath || '').trim().replace(/[\\/]openclaw\.json$/i, '');
const getProfileProviderAliases = (profileId, profile) => {
    const provider = String(profile?.provider || '').toLowerCase();
    ...
    if (id)
        aliases.add(id.split(':')[0]);
    return Array.from(aliases).filter(Boolean);
};
const getChoiceAliases = (authChoice) => AUTH_CHOICE_PROVIDER_ALIASES[String(authChoice || '').trim()] || [String(authChoice || '').trim()];
const providerAliasSets = {
...
const providerMatchesAny = (provider, filters) => {
...
const profileMatchesAliases = (profileId, profile, aliases) => {
    const provider = String(profile?.provider || '').toLowerCase();
    const id = String(profileId || '').toLowerCase();
...
async function getAgentAuthProfilePaths(configDir) {
...
async function collectAuthProfiles(configDir) {
    ...
    const normalizeProfileMeta = (profileId, profile) => {
        const provider = String(profile?.provider || String(profileId).split(':')[0] || '').toLowerCase();
        const mode = String(profile?.mode || profile?.type || '').toLowerCase();
        return { provider, mode };
    };
    const findFallbackGlobalKey = (provider, mode) => {
        if (!provider)
            return '';
        for (const [key, entry] of merged.entries()) {
            ...
            if (!entryProvider || entryProvider !== provider)
                continue;
            if (mode && entryMode && entryMode !== mode)
                continue;
            return key;
        }
        return '';
 ...
 */
async function copyDir(src, dest, progressCallback) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (progressCallback)
                progressCallback(`Copying ${entry.name}...`);
 ...
 */
async function parseSkillMetadata(skillDir, fallbackId) {
    ...
    try {
        ...
        if (match && match[1]) {
            ...
            if (nameMatch)
                defaultMeta.name = nameMatch[1].replace(/['"]/g, '').trim();
            if (descMatch)
                defaultMeta.desc = descMatch[1].replace(/['"]/g, '').trim();
        }
    }
    catch (e) {
 ...
 */
async function scanSkillsInDir(dir) {
    const results = [];
    try {
 ...
 */
async function scanInstalledSkills(...basePaths) {
...
const defaultControlCenterState = () => ({
    tasks: [],
    projects: [],
    queue: [],
    audit: [],
...
const buildId = (prefix) => `${prefix}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
...
async function loadActivityStore() {
    try {
        ...
        if (Array.isArray(parsed))
            activityBuffer = parsed.slice(-ACTIVITY_MAX);
    }
    catch {
        activityBuffer = [];
    }
}
async function flushActivityStore() {
...
function pushActivity(event) {
    const full = { id: buildId('act'), ...event };
    // Deduplicate: skip if exact same title+timestamp already exists (avoid double-scan)
    const isDuplicate = activityBuffer.some(e => e.title === full.title && e.timestamp === full.timestamp);
    if (isDuplicate)
        return full;
    activityBuffer.push(full);
    if (activityBuffer.length > ACTIVITY_MAX)
        activityBuffer = activityBuffer.slice(-ACTIVITY_MAX);
    void flushActivityStore();
    return full;
}
// ── Inference Engine ──────────────────────────────────────────────────────────
// Map raw path/event → semantic type + title
function inferFsEvent(watchEvent, filePath, existed) {
...
async function readLauncherConfigPaths() {
    const fallback = {
...
async function buildWatchDirs() {
    ...
    if (stateDir)
        dirs.push(path.join(stateDir, 'agents'));
    // Remove duplicates and missing dirs
    return [...new Set(dirs)];
}
const activeWatchers = [];
async function startActivityWatcher(extraDirs = []) {
    ...
    for (const dir of dirsToWatch) {
        ...
        try {
            const watcher = fsWatch(dir, { recursive: true }, (watchEvent, filename) => {
                ...
                if (base.startsWith('.') || fullPath.endsWith('~') || fullPath.endsWith('.lock'))
                    return;
                const existed = existsSync(fullPath);
                const inferred = inferFsEvent(watchEvent, fullPath, existed);
                if (inferred)
                    pushActivity({ ...inferred, timestamp: Date.now() });
            });
...
async function scanAllSessions(stateDir) {
...
const cronLastSeen = new Map();
async function scanCronJobs(stateDir) {
    const resolved = stateDir || (await readLauncherConfigPaths()).stateDir;
    const base = resolved || path.join(HOME, '.openclaw');
...
// ─────────────────────────────────────────────────────────────────────────────
async function readControlCenterState() {
...
async function writeControlCenterState(state) {
    await fs.writeFile(CONTROL_CENTER_STATE_FILE(), JSON.stringify(state, null, 2), 'utf-8');
}
async function appendControlAudit(state, action, targetId, ok, message) {
    const entry = {
        id: buildId('audit'),
        action,
        targetId,
...
function buildControlOverview(state) {
...
function buildControlBudgetStatus(state) {
    ...
    const status = usedRatio >= 1 ? 'critical' : usedRatio >= warnRatio ? 'warn' : 'ok';
    return {
...
function isControlMutationCommand(fullCommand) {
    const prefixes = [
        'control:tasks:add ',
...
function parseControlPayload(fullCommand) {
...
async function enforceControlMutationTokenGate(fullCommand) {
...
function pushQueueIfMissing(state, item) {
...
function pushApprovalIfMissing(state, item) {
    const sourceKey = String(item.sourceKey || '').trim();
    if (sourceKey) {
...
async function runControlAutoSync() {
    ...
    if (state.queue.length !== beforeQueue || state.approvals.length !== beforeApprovals) {
        state = await appendControlAudit(state, 'control.autoSync', 'runtime-signals', true, `queue+${state.queue.length - beforeQueue}, approvals+${state.approvals.length - beforeApprovals}`);
    }
    return {
...
ipcMain.handle('shell:exec', async (_event, command, args = []) => {
    const fullCommand = args.length > 0 ? `${command} ${args.join(' ')}` : command;
    if (fullCommand === 'app:check-update') {
        try {
            const current = app.getVersion();
            const releases = await new Promise((resolve, reject) => {
                const req = https.get('https://api.github.com/repos/nt-nerdtechnic/ClawLaunch/releases?per_page=1', { headers: { 'Accept': 'application/vnd.github+json', 'User-Agent': 'NT-ClawLaunch' } }, (res) => {
    ...
    if (fullCommand.startsWith('control:auth:set-token ')) {
        try {
            ...
            const next = { ...state, controlToken: newToken };
    ...
    if (fullCommand.startsWith('control:approvals:add ')) {
        try {
            ...
            const state = await readControlCenterState();
            const next = { ...state, approvals: [item, ...state.approvals] };
    ...
    const getNTTasksFile = async () => {
        const { workspacePath } = await readLauncherConfigPaths();
        if (workspacePath)
            return path.join(workspacePath, 'tasks.json');
    ...
    const writeNTTasks = async (tasks) => {
        const file = await getNTTasksFile();
        ...
        try {
            ...
            const updated = tasks.map((t) => {
                ...
                if (status === 'done') {
                    ...
                    next.components = (t.components || []).map((c) => ({ ...c, progress: 1.0 }));
                }
                return next;
            });
            if (!found)
    ...
    const runSilent = (cmd) => new Promise((resolve) => {
    ...
    if (fullCommand.startsWith('system:crontab:toggle ')) {
        try {
            ...
            if (!raw)
                return { code: 1, stdout: '', stderr: 'raw is required', exitCode: 1 };
            const res = await runSilent('crontab -l');
            const lines = res.stdout.split('\n');
    ...
    if (fullCommand.startsWith('system:launchagents:toggle ')) {
        try {
            ...
            if (!agentLabel)
                return { code: 1, stdout: '', stderr: 'label is required', exitCode: 1 };
            const home = process.env['HOME'] || '';
    ...
    if (fullCommand.startsWith('cron:toggle ')) {
        try {
            ...
            const stateDir = String(payload?.stateDir || process.env['OPENCLAW_STATE_DIR'] || path.join(process.env['HOME'] || '', '.openclaw')).trim();
            if (!jobId)
                return { code: 1, stdout: '', stderr: 'jobId is required', exitCode: 1 };
            const cronPath = path.join(stateDir, 'cron', 'jobs.json');
            const raw = await fs.readFile(cronPath, 'utf-8');
            const data = JSON.parse(raw);
            let toggled = false;
            data.jobs = (data.jobs || []).map((job) => {
                if (job.id === jobId) {
                    toggled = true;
                    return { ...job, enabled: !job.enabled, updatedAtMs: Date.now() };
                }
                return job;
            });
            if (!toggled)
                return { code: 1, stdout: '', stderr: `job ${jobId} not found`, exitCode: 1 };
        ...
        try {
            ...
            const queue = state.queue.map((item) => {
                if (item.id !== itemId)
                    return item;
                found = true;
                return { ...item, status: 'acked', ackedAt: nowIso() };
            });
            if (!found) {
    ...
    if (fullCommand.startsWith('snapshot:read-model')) {
        try {
            ...
            const historyCandidatePaths = Array.isArray(payload?.historyCandidatePaths)
            ...
            const candidatePaths = Array.isArray(payload?.candidatePaths)
            ...
            if (parsed && typeof parsed === 'object') {
                ...
                if (parsed.models && typeof parsed.models.providers === 'object' && parsed.models.providers !== null) {
                    for (const [providerKey, providerVal] of Object.entries(parsed.models.providers)) {
                        if (providerVal && typeof providerVal === 'object' && !Array.isArray(providerVal.models)) {
                            providerVal.models = [];
                            changed = true;
                        }
                    }
    ...
    if (fullCommand === 'skill:import') {
        try {
            ...
            if (result.canceled)
                return { code: 0, stdout: 'Canceled', exitCode: 0 };
    ...
    if (fullCommand.startsWith('auth:add-profile')) {
        try {
            ...
            const hasMatched = listed.profiles.some((profile) => profileMatchesAliases(profile.profileId, { provider: profile.provider }, aliases) && (CREDENTIALLESS_AUTH_CHOICES.has(authChoice) || profile.agentPresent));
            if (!hasMatched) {
    ...
    if (fullCommand.startsWith('config:model-options')) {
        try {
            ...
            const healthyProviders = Array.from(new Set(authOverview.profiles
                .filter((profile) => profile.agentPresent && profile.credentialHealthy)
                .flatMap((profile) => getProfileProviderAliases(profile.profileId, { provider: profile.provider }))));
            ...
            if (corePath) {
                ...
                if ((listRes.code ?? 0) === 0 && String(listRes.stdout || '').trim()) {
                    ...
                    for (const row of rows) {
                        ...
                        if (!provider)
                            continue;
                        if (!providerMatchesAny(provider, effectiveFilters))
                            continue;
                        if (!grouped.has(provider)) {
                            grouped.set(provider, new Set());
                        }
            ...
            for (const modelFile of modelFiles) {
                ...
                for (const [providerKey, providerConfig] of Object.entries(providers)) {
                    ...
                    const resolvedModels = rawModels
    ...
    if (fullCommand.startsWith('project:check-empty')) {
        ...
        try {
            ...
            if (!stats.isDirectory())
                return { code: 1, stderr: 'Not a directory', exitCode: 1 };
    ...
    if (fullCommand.startsWith('project:initialize')) {
        try {
            ...
            const checkAndWrap = async (dirPath, subName) => {
                try {
            ...
            return new Promise((resolve) => {
                let childProcess;
                const runCommandWithStreaming = (cmd, title) => {
                ...
                childProcess.on('close', async (code) => {
                    ...
                    try {
                        ...
                        const trackPreExisting = async (targetPath) => {
                            try {
                        ...
                        const ensureDirWithTracking = async (dirPath) => {
                            await fs.mkdir(dirPath, { recursive: true });
                        ...
                        const setupRes = await runCommandWithStreaming(`zsh -ilc "${setupEnv} pnpm openclaw setup --workspace ${shellQuote(finalWorkspacePath)}" 2>&1 || ${setupEnv} pnpm openclaw setup --workspace ${shellQuote(finalWorkspacePath)}`, 'Initializing OpenClaw config...');
                        if (setupRes.code !== 0) {
...
ipcMain.handle('port:find-free', async (_event, startPort, endPort) => {
...
ipcMain.handle('usage:scan-sessions', async (_event, payload) => {
    try {
        const homeDir = process.env.HOME || process.env.USERPROFILE || '';
        const parsed = payload ? (() => { try {
            return JSON.parse(payload);
        }
        catch {
            return {};
        } })() : {};
        const agentsDir = parsed.agentsDir || path.join(homeDir, '.openclaw', 'agents');
        const events = [];
        let agentIds = [];
        try {
        ...
        for (const agentId of agentIds) {
            ...
            for (const file of files) {
                let content = '';
                try {
                    content = await fs.readFile(path.join(sessionsDir, file), 'utf-8');
                }
                catch {
                    continue;
                }
...
// ──────────────────────────────────────────────────────────────────────────
ipcMain.handle('shell:kill-port-holder', async (_event, rawPort) => {
...
ipcMain.handle('openclaw:chat.invoke', async (_event, request) => {
    ...
    if (!request.stream) {
        ...
        if (runId) {
            ...
            if (state)
                activeChatRequests.set(request.requestId, { ...state, runId });
        }
        const timeoutMs = 65000;
        const startAt = Date.now();
...
ipcMain.handle('openclaw:chat.abort', async (_event, requestId) => {
    ...
    try {
        const runtime = await resolveOpenClawRuntime();
        const abortParams = { sessionKey: chatState.sessionKey };
        if (chatState.runId)
            abortParams.runId = chatState.runId;
        if (chatState.agentId)
            abortParams.agentId = chatState.agentId;
...
// List all openclaw session stubs by scanning agents/*/sessions/*.jsonl
    ...
    try {
        ...
        for (const agentId of agentIds) {
            ...
            for (const file of files) {
                ...
                if (messageCount === 0)
                    continue;
                sessions.push({ sessionKey, agentId, lastMessage, lastTimestamp, messageCount });
            }
        }
        sessions.sort((a, b) => {
...
// Load full message history from a single openclaw session JSONL file
ipcMain.handle('openclaw:session.load', async (_event, payload) => {
    try {
        ...
        const { stateDir } = await readLauncherConfigPaths();
        const base = stateDir || path.join(HOME, '.openclaw');
        const sessionFile = path.join(base, 'agents', agentId, 'sessions', `${sessionKey}.jsonl`);
        let content = '';
        try {
...
// ── Activity Engine IPC ───────────────────────────────────────────────────────
ipcMain.handle('activity:events:list', async (_event, payload) => {
    const opts = payload ? (() => { try {
        return JSON.parse(payload);
    }
    catch {
        return {};
...
ipcMain.handle('activity:scan:now', async (_event, payload) => {
    const opts = payload ? (() => { try {
        return JSON.parse(payload);
    }
    catch {
...
ipcMain.handle('activity:watch:restart', async (_event, payload) => {
    const opts = payload ? (() => { try {
        return JSON.parse(payload);
    }
    catch {
